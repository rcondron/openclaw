  ...
  return {
    ...
    resolved: {
      ...
      profiles: {
        ...
        browserless: { cdpPort: 18800, color: "#FF4500" },
      },
    },
    profiles: new Map(),
  };
...
describe("browser server-context tab selection state", () => {
  it("updates lastTargetId when openTab is created via CDP", async () => {
    ...
    const state = makeState("browserless");
    const ctx = createBrowserRouteContext({ getState: () => state });
    const browserless = ctx.forProfile("browserless");

    const opened = await browserless.openTab("http://127.0.0.1:8080");
    expect(opened.targetId).toBe("CREATED");
    expect(state.profiles.get("browserless")?.lastTargetId).toBe("CREATED");
  ...
  it("blocks unsupported non-network URLs before any HTTP tab-open fallback", async () => {
    ...
    await expect(browserless.openTab("file:///etc/passwd")).rejects.toBeInstanceOf(
