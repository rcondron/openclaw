...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "browserless";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "browserless"}`,
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (openclaw|extension). Default: openclaw. Use extension for TabHR (CDP on port 9220).")
    .action(
